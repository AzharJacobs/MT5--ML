...
try:
    db2 = get_connection()
    db2.connect()
    h1 = db2.fetch_dataframe('SELECT * FROM xauusd_ohlcv WHERE timeframe = %s ORDER BY timestamp ASC', ('1H',))
    h4 = db2.fetch_dataframe('SELECT * FROM xauusd_ohlcv WHERE timeframe = %s ORDER BY timestamp ASC', ('4H',))
    db2.disconnect()
except:
    pass
