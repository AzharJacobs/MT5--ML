...
Observation per step:
  For each timeframe (5min, 15min, 1H, 4H):
  ...
  Total: 4 × 50 × 5 + 3 + 3 = 1006 dims  (vs 255 for the MTF feature env)

Normalisation (per-window, done live):
  Prices  → Z-score using that window's close mean/std
  Volume  → ratio to window mean volume
  This means the agent sees *shape* and *relative movement*, not absolute prices.
  It will generalise across price levels and years without retraining.

SL/TP:
  ATR-based (no zone levels available).
  sl = entry ± sl_atr × ATR14
  tp = entry ± tp_atr × ATR14
  Default sl_atr=1.5, tp_atr=2.5  → RR ≈ 1.67

Reward:
  Same structure as XAUUSDMultiTFEnv — SL penalty, TP bonus, drawdown penalty.
"""
...
class XAUUSDRawEnv(gym.Env):
    ...
    def __init__(
        ...
        window_size: int        = 50,    # bars of lookback per TF
        atr_period: int         = 14,
        sl_atr: float           = 1.5,   # SL distance in ATR units
        tp_atr: float           = 2.5,   # TP distance in ATR units
        ...
        # Precompute ATR for every bar (avoids live recalculation in step())
        self._primary_atr = self._compute_atr(self.primary_df, atr_period)

        # Timestamp alignment (same logic as MultiTFEnv)
        self._aligned = self._precompute_alignment()

        # Observation: 4 TFs × window_size × 5 OHLCV features + time + position
    ...
    def _compute_atr(df: pd.DataFrame, period: int) -> np.ndarray:
        """Precompute rolling ATR (Wilder's) for every bar. Returns array aligned to df."""
        ...
        # Fill leading NaNs with the first valid ATR
        first_valid = atr[atr > 0]
        fill = float(first_valid[0]) if len(first_valid) > 0 else 1.0
    ...
    # ─────────────────────────────────────────────────────────────────────────
    #  Timestamp alignment (primary → secondary TFs)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _window_features(self, df: pd.DataFrame, end_idx: int) -> np.ndarray:
        """
        Extract and normalise a window of `window_size` candles ending at end_idx.
        Returns a flattened (window_size × 5) array.
        Prices are Z-scored with the window's own close mean/std.
        Volume is normalised to the window mean.
        """
        ws    = self.window_size
        ...
        vol_mean = float(vol.mean()) + 1e-9
        vol_norm = (vol / vol_mean).reshape(-1, 1)

        features = np.hstack([price, vol_norm])   # (actual_len, 5)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # Time context (normalised to [0, 1])
    ...
    def _compute_sl_tp(self, step: int, direction: int):
        """Returns (sl, tp, rr) based on ATR. No zone information needed."""
    ...
    def reset(self, seed: Optional[int] = None, options=None):
        ...
        # Start after window_size so the first observation is always a full window
        max_start = max(self.window_size, len(self.primary_df) - self.episode_length - 1)
        lo = self.window_size
        hi = max(lo, max_start)
    ...
    def step(self, action: int):
        ...
        reward -= 0.001   # per-step activity cost
    ...
    def render(self):
        row = self.primary_df.iloc[self.current_step]
        atr = float(self._primary_atr[self.current_step])
        print(
            f"step={self.current_step} pos={self.position:+d} "
            f"eq={self.equity:.2f} dd={self.max_drawdown:.2%} "
            f"trades={len(self.trade_history)} close={row['close']:.2f} atr={atr:.3f}"
        )
