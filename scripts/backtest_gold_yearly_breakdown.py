#!/usr/bin/env python3
"""
XAUUSD Gold ZZ backtest: 2023-2025 — month-by-month breakdown.
Runs as one continuous simulation (equity carries between years).
Reports: monthly PnL, WR, lot sizes, and per-trade detail.
"""
...
def _print_year_table(df_year: pd.DataFrame, year: int, prev_equity: float) -> float:
    ...
    W = 80
    print(f"\n{'='*W}")
    print(f"  GOLD ZZ  |  XAUUSD  |  {year}  |  Start: ${prev_equity:,.2f}")
    print(f"{'─'*W}")
...
def _print_trade_detail(df_all: pd.DataFrame) -> None:
    ...
    print(f"  {'#':>4}  {'Date':<17} {'Side':<5} {'Lot':>6} {'Entry':>9} "
          f"{'SL':>9} {'TP':>9} {'Exit':>9} {'PnL':>9} {'Result':<8}")
    print(f"{'─'*W}")
    for idx, row in df_all.iterrows():
        outcome_str = "WIN" if row["outcome"] == 1 else ("LOSS" if row["outcome"] == -1 else "EXPIRED")
        ...
        print(f"  {idx+1:>4}  {date_str:<17} {row['side']:<5} {row['lot']:>6.3f} "
              ...
              f"{row['exit']:>9.2f} {sign}{row['pnl']:>8.2f} {outcome_str:<8}")
    print(f"{'='*W}")
...
def main():
    ...
    print("\n" + "="*80)
    print(f"  GOLD ZZ Backtest  |  XAUUSD  |  2023–2025  |  ${cash:.2f} starting cash")
    print(f"  Config: make_gold_config() — same as live bot")
    print("="*80)
    print("  Running simulation (may take a few minutes)...")

    cfg = make_gold_config()

    _, df_all = run_backtest_gold(
        cfg=cfg,
        start="2023-01-01",
        end="2025-12-31",
        cash=cash,
